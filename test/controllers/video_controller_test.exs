defmodule Rumbl.VideoControllerTest do
  ...
  test "requires user authentication on all actions", %{conn: conn} do
    Enum.each([
      ...
      get(conn, video_path(conn, :edit, "124")),
  ...
  alias Rumbl.Video
  alias Rumbl.Repo
  @valid_attrs %{url: "http://yooutu.be", title: "vid", description: "a vid"}
  ...
  @tag login_as: "max"
  test "does not create  a user video and renders errors when invalid", %{conn: conn} do
  ...
  test "authorizes actions against access by other users", %{user: owner, conn: conn} do
    ...
    non_onwer = insert_user(username: "sneaky")
    conn = assign(conn, :current_user, non_onwer)
